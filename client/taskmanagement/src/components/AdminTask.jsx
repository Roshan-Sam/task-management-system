...
import { Label, Badge, Textarea, Button, Toast, Alert } from "flowbite-react";
...
const AdminTask = () => {
  ...
  const fetchTask = async (taskId) => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL}admin/getsingletaskadmin/${taskId}/`,
        {
  ...
  const fetchComments = async (taskId) => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL}admin/getcomments/${taskId}/`,
        {
          headers: { Authorization: `Bearer ${Cookies.get("token")}` },
        }
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_API_URL}admin/addcomment/`,
  ...
  const handleDelete = async (id) => {
    try {
      const res = await axios.delete(
        `${import.meta.env.VITE_API_URL}admin/deletecomment/${id}/`,
        {
          headers: { Authorization: `Bearer ${Cookies.get("token")}` },
        }
      );
        ...
        <div className="w-full shadow-sm-light shadow-slate-200 border-1 p-4 flex flex-col gap-4 items-center">
          ...
          <div className="flex flex-col gap-2 items-center">
            ...
            <div className="max-w-[200px] flex justify-between items-center gap-4">
              {task.created_by && (
                <>
                  <p className="font-light text-lg">@{currentUser.username}</p>
                  <img
          ...
          <div className="flex flex-col gap-2 items-center">
            ...
            {task.status === "Pending" ? (
              ...
              <Badge className="w-fit text-sm" color="success">
                {task.status}
              </Badge>
            )}
          </div>
