...
const EditTask = () => {
  ...
  useEffect(() => {
    ...
    const idFromUrl = urlParams.get("tid");
    if (idFromUrl) {
      fetchTask(idFromUrl);
      setTaskId(idFromUrl);
    }
  ...
  const fetchTask = async (taskId) => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_URL}admin/gettask/${taskId}/`,
        {
          headers: {
      ...
      const formattedDueDate = taskData.due_date.split("T")[0];
      setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.put(
        `${import.meta.env.VITE_API_URL}admin/edittask/${taskId}/`,
        formData,
        {
          headers: {
            Authorization: `Bearer ${Cookies.get("token")}`,
  ...
  return (
    <div className="max-w-lg mx-auto p-3 w-full flex flex-col items-center pt-20">
      ...
      <h1 className="text-2xl font-semibold">Edit a task</h1>
